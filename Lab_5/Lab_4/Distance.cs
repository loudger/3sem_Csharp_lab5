...
namespace DistanceLibrary
{
    public static class Distance
    {
        public static int CalculateDistance(string str1Param, string str2Param)
        {
            ...
            //возвращается длина другой строки
            ...
            //Инициализация нулевой строки и нулевого столбца матрицы
            for (int i = 0; i <= str1Len; i++) matrix[i, 0] = i;
            for (int j = 0; j <= str2Len; j++) matrix[0, j] = j;
            //Вычисление расстояния Дамерау-Левенштейна
            for (int i = 1; i <= str1Len; i++)
            {
                for (int j = 1; j <= str2Len; j++)
                {
                    ...
                    int subst = matrix[i - 1, j - 1] + symbEqual; //Замена
                                                                  ...
                                                                  //как минимальный из трех случаев
